    ...
    public partial class SiparisEmriDuzenle : MetroFramework.Forms.MetroForm
    {
        ...
        private void btn_duzenle_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (rbGelen.Checked)
                {
                    db.UpdateSE(Convert.ToInt32(lbl_id.Text), Convert.ToString(cmb_proje_no.Text), Convert.ToString(txt_siparis_no.Text), Convert.ToString(cmb_tedarikci.Text), Convert.ToString(txt_talepKisi.Text), Convert.ToDateTime(date_teslim.Text), Convert.ToInt32(txt_vade.Text), Convert.ToDateTime(date_temin.Text), Convert.ToDecimal(txt_fiyat.Text), Convert.ToString(cmb_birim.Text), Convert.ToDecimal(siparis_euro), Convert.ToDecimal(siparis_dolar), Convert.ToDecimal(siparis_tl), Convert.ToString(rcb_aciklama.Text), Convert.ToString("Gelen"));
                }
